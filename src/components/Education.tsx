...
import { FaGraduationCap, FaCertificate, FaTrophy, FaFreeCodeCamp, FaHackerrank, FaCode } from 'react-icons/fa';
...
export default function Education() {
    const getIssuerIcon = (issuer: string) => {
        switch (issuer.toLowerCase()) {
            case 'freecodecamp':
                return <FaFreeCodeCamp className="text-primary-400" />;
            case 'hackerrank':
                return <FaHackerrank className="text-secondary-400" />;
            default:
                            ...
                            >
                                <Card className="p-4">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        {cert.issueDate && (
                                            <span className="text-[10px] text-primary-400 font-medium">{cert.issueDate}</span>
                                        )}
                                    </div>
                                </Card>
                            </motion.div>
